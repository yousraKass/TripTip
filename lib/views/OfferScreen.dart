      ...
      body: FutureBuilder<OfferData>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(child: Text('Error loading offer details'));
          }
...
class OfferDetailsSection extends StatelessWidget {
  ...
  const OfferDetailsSection({required this.offerData});
...
class _ReviewSectionState extends State<ReviewSection> {
  ...
  Widget _buildRatingBar(String label, double value) {
    return Row(
      children: [
        Text(label),
        Icon(Icons.star, color: AppColors.main, size: 16),
        SizedBox(width: 10),
        Expanded(
          child: ClipRRect(
  ...
  Widget _buildReviewsHeader(int reviewCount) {
    return Row(
      ...
      children: [
        ...
        InkWell(
          onTap: () {
            Navigator.pushNamed(context, ReviewScreen.pageRoute);
          },
          child: Text(
            AppTexts.viewAll,
            style: TextStyle(
...
class OfferImage extends StatelessWidget {
  ...
  OfferImage({required this.imageUrl});
